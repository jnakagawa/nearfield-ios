...
const server = http.createServer((req, res) => {
  ...
  if (p.endsWith('/status')) {
    ...
    res.end(html);
  } else if (p === '/simulator' || p === '/explainer' || p.includes('/simulator/')
             || p.includes('/config/') || p.includes('/explainer')) {
